...
def _create_card_from_constants(config, badge_text=BADGE_TEXT['learn_more']):
    ...
    return _create_card(config['url'], config['logo'], config['alt'], config['name'], badge_text=badge_text)

def get_certification_cards():
    """Get certification cards from constants"""
    return [
        _create_card(cert['url'], cert['logo'], cert['alt'], cert['title'], cert['subtitle'], BADGE_TEXT['view_badge_icon'])
...
def get_education_cards():
    ...
    for program in EDUCATION_PROGRAMS:
        ...
        cards.append(_create_card(
            institution['url'], institution['logo'], institution['alt'],
            program['program'], f"{institution['name']} • {program['years']}"
        ))
    return cards
...
def get_connect_cards():
    ...
    return [
        _create_card(social['url'], social['logo'], social['alt'], social['name'], social['description'])
        for social in SOCIAL_LINKS.values()
    ]

def get_frontend_tech_cards():
    """Get frontend tech cards from constants"""
    return [_create_card_from_constants(tech) for tech in TECHNOLOGIES.values() if tech['category'] == 'frontend']

def get_backend_tech_cards():
    """Get backend tech cards from constants"""
    return [_create_card_from_constants(tech) for tech in TECHNOLOGIES.values() if tech['category'] == 'backend']

def get_infra_tech_cards():
    """Get infrastructure tech cards from constants"""
    return [_create_card_from_constants(tech) for tech in TECHNOLOGIES.values() if tech['category'] == 'infra']
